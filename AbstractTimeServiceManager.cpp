...
{
	bool status = false;
	get(runningTimeEntryUrl(userId, m_workspaceId), false, [this, &status](QNetworkReply *rep) {
		try
		{
			status = jsonToRunningTimeEntry(json::parse(rep->readAll())).isValid();
		}
...
QDateTime AbstractTimeServiceManager::stopRunningTimeEntry(const QString &userId, bool async)
{
	...
	t.setEnd(now);
	patch(timeEntriesUrl(userId, m_workspaceId), QByteArray::fromStdString(timeEntryToJson(t).dump()), async);
	return now;
}
...
{
	json j;
	get(runningTimeEntryUrl(userId, m_workspaceId), false, [&j](QNetworkReply *rep) {
		try
		{
...
void AbstractTimeServiceManager::startTimeEntry(const QString &userId, const QString &projectId, const QString &description, bool async)
{
	startTimeEntry(userId, projectId, description, QDateTime::currentDateTimeUtc(), async);
}

void AbstractTimeServiceManager::startTimeEntry(const QString &userId, const QString &projectId, const QDateTime &start, bool async)
{
	startTimeEntry(userId, projectId, QString{}, start, async);
}

void AbstractTimeServiceManager::startTimeEntry(const QString &userId, const QString &projectId, const QString &description, const QDateTime &start, bool async)
{
	Project p;
	auto it = std::find_if(m_projects.begin(), m_projects.end(), [&projectId](const Project &p) { return p.id() == projectId; });
	if (it != m_projects.end())
		p = *it;
	else
		p = Project{projectId, projectName(projectId)};
	TimeEntry t{{}, p, description, userId, start, {}};
	post(timeEntriesUrl(userId, m_workspaceId), QByteArray::fromStdString(timeEntryToJson(t).dump()), async, 201);
}
                                                              ...
                                                              [[maybe_unused]] std::optional<int> pageSize)
{
	...
	if (supportedPagination().testFlag(Pagination::TimeEntries))
	{
		...
		{
			QUrlQuery query;
			query.addQueryItem(timeEntriesPageSizeHeaderName(), QString::number(pageSize.value_or(timeEntriesPaginationPageSize())));
...
//			std::transform(j.begin(), j.end(), workspaces.begin(), std::bind(&AbstractTimeServiceManager::jsonToWorkspace, this, _1));
...
{
	// TODO: validate this
//	auto it = std::find_if(m_workspaces.begin(), m_workspaces.end(), [&workspaceId](Workspace &w) { return w.id() == workspaceId; });
...
{
	get(currentUserUrl(), false, [this](QNetworkReply *rep) {
		try
		{
			json j{json::parse(rep->readAll().toStdString())};
	...
	auto worker = [this, &url]() -> bool {
		...
		get(url, [this, &retCode](QNetworkReply *rep) {
			try
			{
				...
				using namespace std::placeholders;
//				std::transform(j.begin(), j.end(), m_users.begin(), std::bind(&AbstractTimeServiceManager::jsonToUserData, this, _1));
...
void AbstractTimeServiceManager::updateProjects()
{
	...
	auto url = projectsUrl(workspaceId());
	auto worker = [this, &url]() -> bool {
		bool retVal;
